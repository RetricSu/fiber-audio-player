...
export function createPodcastCommands() {
    ...
    podcast
        ...
        try {
            ...
            switch (format) {
                case 'json':
                    output = JSON.stringify(podcasts.map((p) => ({
                        ...p,
                ...
                case 'csv':
                    ...
                    output += podcasts
                        .map((p) => `"${p.id}","${p.title.replace(/"/g, '""')}",${episodeCounts[p.id] || 0},${formatDate(p.created_at)}`)
                        .join('\n');
                    break;
                case 'table':
                default: {
    ...
    podcast
        ...
        try {
            ...
            if (options.description !== undefined)
                updateData.description = options.description.trim();
