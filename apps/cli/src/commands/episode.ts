...
import { ConfigError, EpisodeStatus, ApiError } from '../lib/types.js';
import { formatError, formatDuration, formatFileSize, formatTimestamp, validateFile } from '../lib/utils.js';

export function createEpisodeCommands(): Command {
  const episode = new Command('episode');
  episode.description('Episode management commands');

  // Helper to create API client with config
  async function createClient(): Promise<AdminApiClient> {
    ...
    .action(async (options: {
      ...
      try {
        ...
        if (options.file) {
          ...
          const uploadResult = await client.uploadEpisodeAudio(
            episode.id,
            fileBuffer,
            fileName,
            validation.mimeType!
  ...
  episode
    ...
    .action(async (options: {
      ...
      try {
        ...
        // Validate status
        const validStatuses: EpisodeStatus[] = ['draft', 'processing', 'ready', 'published'];
        const statusFilter = options.status === 'all' ? null : options.status as EpisodeStatus;
